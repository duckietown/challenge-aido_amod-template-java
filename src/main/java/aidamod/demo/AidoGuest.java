 ...
 * 
 * Usage: java -cp target/amod-VERSION.jar amod.aido.demo.AidoGuest [IP of host]
 */
public class AidoGuest {
	...
	/**
	 * @param ip
	 *            for instance "localhost"
	 */
	public AidoGuest(String ip) {
		this.ip = ip;
	}

	public void run(String scenario, int requestsDesired, int numberOfVehicles)
			throws UnknownHostException, IOException, Exception {
		/** connect to AidoGuest */
		try (StringSocket stringSocket = new StringSocket(new Socket(ip, PORT))) {

			/** send initial command, e.g., {SanFrancisco.20080518} */
			Tensor config = Tensors
					.of(StringScalar.of(scenario)); /** scenario name */
			stringSocket.writeln(config);

			/**
			 * receive information on chosen scenario, i.e., bounding box and
			 * number of requests, the city grid is inside the WGS:84
			 * coordinates bounded by the box bottomLeft, topRight, {{longitude
			 * min, latitude min}, {longitude max, latitude max}}
			 */
			...
			while (true) {
				...
				if (Objects.nonNull(string)) { // when the server closed
												// prematurely
					Tensor status = Tensors.fromString(string);
					if (Tensors.isEmpty(status)) // server signal that
